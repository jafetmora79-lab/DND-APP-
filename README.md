...
For a plain-language walkthrough of the whole night (login → prep → placing players on an encounter → live table → combat → won/lost), see [HOW-THE-TABLE-WORKS.txt](HOW-THE-TABLE-WORKS.txt). What is still thin on the DM vs player loop (how fights start, how rounds turn over, how they finish) is in [WHAT-IS-MISSING.txt](WHAT-IS-MISSING.txt).
