...
function splitProfs(list: SrdMonster['proficiencies']) {
  ...
  for (const p of list ?? []) {
    ...
    if (name.startsWith('Saving Throw:')) saves.push(`${name.replace('Saving Throw: ', '')} ${signed}`)
    else if (name.startsWith('Skill:')) skills.push(`${name.replace('Skill: ', '')} ${signed}`)
  }
...
export function mapSrdMonster(m: SrdMonster) {
