...
export function PartyOverview({ characters, selectedId, onSelectCharacter, onClose, canEdit = false }: Props) {
  const [notes, setNotes] = useState(() => {
    const saved = localStorage.getItem('dnd-party-notes')
    return saved || ''
  })
        ...
        {!viewingSheet ? (
          <>
            <div className="mt-4 space-y-3">
              <h3 className="text-xs uppercase tracking-wider text-gold font-semibold">Party Members</h3>
              <div className="space-y-2">
                {characters.map((char) => {
                  const hpPercent = getHpPercent(char.sheet.hpCurrent, char.sheet.hpMax)
                  ...
                  return (
                    <div
                      key={char.id}
                      className={cn(
                        'rounded-lg border p-3 transition-all hover:border-gold/50 cursor-pointer',
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {isDying && <Activity className="h-4 w-4 text-blood animate-pulse" />}
                          </div>
                          <div className="mt-1 text-xs text-muted">
                            Level {char.sheet.level}
                          </div>
                        </div>
                        <Button
                          size="sm"
                      ...
                      <div className="mt-2">
                        <div className="flex items-center justify-between text-xs mb-1">
                          ...
                          <span className={cn('font-medium', isDying && 'text-blood', isDead && 'text-blood')}>
                            {char.sheet.hpCurrent}/{char.sheet.hpMax}
                          </span>
                        </div>
                        <div className="h-1.5 w-full overflow-hidden rounded-full bg-bg">
                          <div
                            className={cn('h-full rounded-full transition-all', getHpColor(hpPercent))}
                            style={{ width: `${hpPercent}%` }}
                          />
                        </div>
                      </div>

                      {char.sheet.conditions && char.sheet.conditions.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-1">
                          {char.sheet.conditions.slice(0, 3).map((cond) => (
                            <span
                              key={cond}
                              className="rounded-md px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-bg font-medium bg-line/70"
                            >
                              {cond}
                            </span>
                          ))}
                          {char.sheet.conditions.length > 3 && (
                            <span className="text-[10px] text-muted">+{char.sheet.conditions.length - 3}</span>
                          )}
                        </div>
                      )}
                ...
                {characters.length === 0 && (
                  <p className="text-sm text-muted">No characters in this campaign yet.</p>
                )}
              </div>
            </div>

            <div className="mt-6 space-y-3">
              <h3 className="text-xs uppercase tracking-wider text-gold font-semibold">Campaign Notes</h3>
              <div className="rounded-lg border border-line bg-panel/50 p-3">
                <Input
                  as="textarea"
                  className="min-h-[100px] w-full resize-none rounded-md border border-line bg-bg px-3 py-2 text-sm focus:border-gold focus:outline-none"
          ...
          <>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pt-3">
              {viewing && (
                <CharacterSheet
                  character={viewing}
                  canEdit={canEdit && viewing.id === selectedId}
                  onChange={(patch) => {
                    // Would need to pass this through from parent
                    console.log('Patch character:', patch)
                  }}
                />
              )}
            </div>
