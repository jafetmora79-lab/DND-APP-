...
export function MapMaker({ map, onChange, onClose, onDeleted }: Props) {
  ...
  async function flush() {
    ...
    try {
      ...
      setMsg(t('mapMaker.mapSaved'))
    } catch (e) {
      const text = e instanceof Error ? e.message : t('mapMaker.couldNotSave')
      setMsg(text)
  ...
  async function attachBackground(file: File) {
    try {
      ...
      setMsg(t('mapMaker.backgroundAttached'))
    } catch (e) {
      setMsg(e instanceof Error ? e.message : t('mapMaker.couldNotUpload'))
    }
  }

  async function clearBackground() {
    patch({ imageUrl: '' })
    setMsg(t('mapMaker.backgroundRemoved'))
  }
      ...
      <div className="rounded-xl border border-line bg-panel p-4">
        ...
        <div className="mt-3 grid gap-3">
          ...
          <div className="rounded-md border border-line p-2">
            ...
            <p className="mt-1 text-xs text-muted">{t('mapMaker.backgroundHint')}</p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant="ghost"
              onClick={() => {
                if (!confirm(t('mapMaker.deleteConfirm'))) return
