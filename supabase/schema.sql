...
create table if not exists public.bestiary_monsters (
  ...
  attacks_per_action int not null default 1,
...
-- Grapple/Shove: the contest (attacker's roll vs. target's static defense) is
-- computed client-side, same reasoning as Hide — this just persists the
-- outcome: locks the attacker's chosen slot and, on success, tags the target
-- Grappled or Prone.
